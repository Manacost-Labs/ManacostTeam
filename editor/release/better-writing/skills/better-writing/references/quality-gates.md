...
For file-backed, high-risk work, run `python3 scripts/check_preservation.py source.md revision.md --gate` and complete the semantic comparison manually. Use `--strict-additions` only when every new numeric token is disallowed; new list numbers and other formatting tokens otherwise remain warnings.
...
Apply the full humanisation criteria when humanisation, formulaic language, genericity, or loss of voice is part of the diagnosed problem. For ordinary drafting and editing, use only the first, eighth, ninth, and tenth criteria plus a brief check that the revision introduced no assistant residue or unsupported ceremony.
...
For Russian prose, use `references/russian-profile.md`; do not apply the English scanner as a release gate.
