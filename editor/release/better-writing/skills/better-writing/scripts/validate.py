...
"""
    ...
    additional_reference = manifest_consistency_errors(
        manifest,
        "2.2.0",
    ...
    return {
        ...
        "allows_reference_discovery_outside_frontmatter": not additional_reference,
...
def validate_preservation_checker(state: ValidationState) -> None:
    """Run deterministic English and Russian fidelity checks."""

    suite = check_preservation.run_self_tests()
    checks = suite.get("checks", {})
    if not isinstance(checks, dict):
        state.errors.append("Preservation checker returned an invalid checks payload")
        return
    state.metrics["preservation_checks"] = len(checks)
    state.metrics["preservation_checks_passed"] = sum(1 for passed in checks.values() if passed)
    if not suite.get("passed"):
        failures = [name for name, passed in checks.items() if not passed]
        state.errors.append(f"Preservation checker self-test failed: {', '.join(failures)}")


def validate_corpus_builder(state: ValidationState) -> None:
    """Run deterministic manifest, deduplication, and chunking checks."""

    suite = build_corpus_manifest.run_self_tests()
    checks = suite.get("checks", {})
    if not isinstance(checks, dict):
        state.errors.append("Corpus builder returned an invalid checks payload")
        return
    state.metrics["corpus_builder_checks"] = len(checks)
    state.metrics["corpus_builder_checks_passed"] = sum(1 for passed in checks.values() if passed)
    if not suite.get("passed"):
        failures = [name for name, passed in checks.items() if not passed]
        state.errors.append(f"Corpus builder self-test failed: {', '.join(failures)}")


def validate_project_profile_support(state: ValidationState) -> None:
    """Validate reusable project-profile behavior and the shipped template."""

    suite = validate_project_profile.run_self_tests()
    checks = suite.get("checks", {})
    if not isinstance(checks, dict):
        state.errors.append("Project-profile validator returned an invalid checks payload")
        return
    state.metrics["project_profile_checks"] = len(checks)
    state.metrics["project_profile_checks_passed"] = sum(1 for passed in checks.values() if passed)
    if not suite.get("passed"):
        failures = [name for name, passed in checks.items() if not passed]
        state.errors.append(f"Project-profile validator self-test failed: {', '.join(failures)}")
    template = state.root / "templates" / "project-profile.json"
    try:
        payload = json.loads(template.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc:
        state.errors.append(f"Project-profile template is invalid JSON: {exc}")
        return
    result = validate_project_profile.validate_profile(payload)
    if not result.get("valid"):
        state.errors.extend(f"Project-profile template: {error}" for error in result.get("errors", []))


def validate_claim_auditor(state: ValidationState) -> None:
    """Validate claim-ledger behavior and the shipped JSONL template."""

    suite = check_claim_coverage.run_self_tests()
    checks = suite.get("checks", {})
    if not isinstance(checks, dict):
        state.errors.append("Claim auditor returned an invalid checks payload")
        return
    state.metrics["claim_audit_checks"] = len(checks)
    state.metrics["claim_audit_checks_passed"] = sum(1 for passed in checks.values() if passed)
    if not suite.get("passed"):
        failures = [name for name, passed in checks.items() if not passed]
        state.errors.append(f"Claim auditor self-test failed: {', '.join(failures)}")
    template = state.root / "templates" / "claim-ledger.jsonl"
    result = check_claim_coverage.audit_path(template)
    if not result.get("passed"):
        state.errors.extend(f"Claim-ledger template: {error}" for error in result.get("errors", []))


def validate_forward_eval_harness(state: ValidationState) -> None:
    """Validate executable forward-eval cases and the local grading harness."""

    path = state.root / "evals" / "forward-evals.json"
    if not path.is_file():
        state.errors.append("Missing required file: evals/forward-evals.json")
        return
    try:
        payload = run_forward_evals.load_manifest(state.root)
    except (json.JSONDecodeError, OSError, ValueError) as exc:
        state.errors.append(f"Forward eval manifest is invalid: {exc}")
        return
    errors = run_forward_evals.validate_manifest(state.root, payload)
    state.errors.extend(f"Forward eval: {error}" for error in errors)
    cases = payload.get("cases", [])
    if isinstance(cases, list):
        state.metrics["forward_eval_count"] = len(cases)
    baseline_dir = payload.get("baseline_dir")
    baseline_results = []
    if isinstance(baseline_dir, str):
        baseline_root = (state.root / baseline_dir).resolve()
        for case in cases if isinstance(cases, list) else []:
            if not isinstance(case, dict) or not isinstance(case.get("id"), str):
                continue
            output_path = baseline_root / f"{case['id']}.md"
            if not output_path.is_file():
                state.errors.append(f"Forward eval baseline is missing: {output_path.relative_to(state.root)}")
                continue
            result = run_forward_evals.grade_case(
                state.root,
                case,
            ...
            if not result.passed:
                state.errors.append(
                    f"Forward eval baseline failed for {result.case_id}: " + "; ".join(result.failures)
                )
    state.metrics["forward_baseline_cases"] = len(baseline_results)
    state.metrics["forward_baseline_cases_passed"] = sum(1 for result in baseline_results if result.passed)
    suite = run_forward_evals.run_self_tests()
    checks = suite.get("checks", {})
    if not isinstance(checks, dict):
        state.errors.append("Forward eval harness returned an invalid checks payload")
        return
    state.metrics["forward_harness_checks"] = len(checks)
    state.metrics["forward_harness_checks_passed"] = sum(1 for passed in checks.values() if passed)
    if not suite.get("passed"):
        failures = [name for name, passed in checks.items() if not passed]
        state.errors.append(f"Forward eval harness self-test failed: {', '.join(failures)}")
...
def validate_skill(skill_path: str) -> dict[str, object]:
    ...
    state = ValidationState(
        ...
        metrics={
            ...
            "preservation_checks_passed": 0,
            "corpus_builder_checks": 0,
            "corpus_builder_checks_passed": 0,
            "project_profile_checks": 0,
            "project_profile_checks_passed": 0,
            "claim_audit_checks": 0,
            "claim_audit_checks_passed": 0,
            "forward_eval_count": 0,
            "forward_harness_checks": 0,
            "forward_harness_checks_passed": 0,
            "forward_baseline_cases": 0,
            "forward_baseline_cases_passed": 0,
        },
    )
    ...
    required_files = (
        ...
        "scripts/probe_better_writing.py", "scripts/scan_aiisms.py", "scripts/check_preservation.py",
        "scripts/build_corpus_manifest.py",
        "scripts/validate_project_profile.py", "scripts/check_claim_coverage.py",
        "scripts/run_forward_evals.py", "scripts/validate.py", "scripts/test_skill.py",
        "assets/aiisms.json", "evals/evals.json", "evals/trigger-evals.json", "evals/forward-evals.json",
        "templates/project-profile.json", "templates/source-pack.json", "templates/claim-ledger.jsonl",
    )
        ...
        for reference in reference_paths:
            ...
            for relative in extract_file_references(reference_content):
                ...
                if not status.contained:
                    ...
                    state.errors.append(f"Referenced regular file does not exist: {relative}")
        linked_references = frozenset(
            Path(relative).relative_to("references").with_suffix("").as_posix()
            for relative in extract_file_references(content)
            if relative.startswith("references/") and relative.endswith(".md")
        )
        unlinked_references = sorted(available_references - linked_references)
        if unlinked_references:
            state.errors.append(
                "Reference files are not discoverable from SKILL.md: " + ", ".join(unlinked_references)
            )
    ...
    validate_preservation_checker(state)
    validate_corpus_builder(state)
    validate_project_profile_support(state)
    validate_claim_auditor(state)
    validate_forward_eval_harness(state)
